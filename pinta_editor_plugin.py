...
class PintaEditorPlugin(TG3Plugin):
    ...
    def _guess_pinta_path(self):
        ...
        candidates = [
            os.path.expandvars(r"%LOCALAPPDATA%\Programs\Pinta\Pinta.exe"),
            os.path.expandvars(r"%ProgramFiles%\Pinta\Pinta.exe"),
            os.path.expandvars(r"%ProgramFiles(x86)%\Pinta\Pinta.exe"),
            "/usr/bin/pinta",
            "/usr/local/bin/pinta",
        ]
        for c in candidates:
